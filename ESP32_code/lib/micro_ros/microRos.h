...
public:
    MicroRos(const MicroRos &) = delete;
    MicroRos &operator=(const MicroRos &) = delete;

    static MicroRos &get_instance();
    void init(const std::string &node_name, const std::string &wifi_name, const std::string &wifi_passward, const std::string &ip, const uint16_t port);
    void start_task();
    void stop_task();
...
private:
    ...
    static void motion_params_service_callback(const void *req, void *res);
    static void timer_callback(rcl_timer_t *timer, int64_t last_call_time);
    static void microros_task(void *args);
