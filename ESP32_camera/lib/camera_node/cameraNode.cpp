...
bool CameraNode::init() {
    ...
    if (!serial_msg_publisher_initialized_) {
        ...
        if (ret != RCL_RET_OK) {
            Serial.printf("serial_msg_publisher_:rclc_publisher_init_default:%d\n", ret);
            return false;
        }
...
rcl_publisher_t *CameraNode::get_serial_msg_publisher() {
...
void _serial_print(const std::string &msg) {
    ...
    if (CameraNode::get_instance().is_connected()) {
        ...
        rcl_ret_t ret;
        ret = rcl_publish(CameraNode::get_instance().get_serial_msg_publisher(), &ros_msg, NULL);
        if (ret != RCL_RET_OK) {
