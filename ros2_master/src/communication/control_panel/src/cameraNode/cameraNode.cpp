...
CameraNode::CameraNode(const std::string &node_name, const std::string &node_nammspace, QObject *parent) : QThread(parent), node_(std::make_shared<rclcpp::Node>(node_name, node_nammspace)) {
    ...
    camera_settings_client_ = node_->create_client<CameraSettingsSrv>(constructNodeName(esp32_camera_node_namespace, esp32_camera_settings_service_name)); // 创建服务客户端
    image_subscription_ = node_->create_subscription<sensor_msgs::msg::CompressedImage>(constructNodeName(esp32_camera_node_namespace, esp32_camera_image_topic_name), reliable_qos, std::bind(&CameraNode::recv_image_msg, this, std::placeholders::_1));
}
