...
MotionWidget::MotionWidget(QWidget *parent)
    ...
    connect(ui->horizontalSlider_linear_speed_percent, &QSlider::sliderReleased, this, &MotionWidget::set_speed_percent);
    connect(ui->horizontalSlider_angular_speed_percent, &QSlider::sliderReleased, this, &MotionWidget::set_speed_percent);

    connect(ui->checkBox_spdPlan_enable, &QCheckBox::stateChanged, this, &MotionWidget::set_speed_plan_state);
...
QWidget *MotionWidget::_initCustomPlot() {
    ...
    QVector<QPair<QColor, QString>> wheelSpeedColorVector = {
        {colorVector[0], "左轮实际速度"},
        {colorVector[1], "左轮目标速度"},
        {colorVector[2], "右轮实际速度"},
        {colorVector[3], "右轮目标速度"},
    };
...
void MotionWidget::on_gamepad_button_clicked() {
    if (!gamepad_->buttonX() && gamepad_->buttonY() && !gamepad_->buttonA() && !gamepad_->buttonB()) {
        ui->pushButton_move_front->click();
    } else if (!gamepad_->buttonX() && !gamepad_->buttonY() && gamepad_->buttonA() && !gamepad_->buttonB()) {
...
void MotionWidget::set_speed_percent() {
    ...
    request->linear_speed_percent = ui->horizontalSlider_linear_speed_percent->value() * 1.0 / ui->horizontalSlider_linear_speed_percent->maximum();
    request->angular_speed_percent = ui->horizontalSlider_angular_speed_percent->value() * 1.0 / ui->horizontalSlider_angular_speed_percent->maximum();
    _update_speed_percent_label(request->linear_speed_percent, request->angular_speed_percent);
...
void MotionWidget::on_write_params() {
    ...
    if (ui->checkBox_left_motor_enable->isChecked())
        request->motor_enable_flags |= 0x01;
    if (ui->checkBox_right_motor_enable->isChecked())
    ...
    request->drivers_settings[0].p = ui->doubleSpinBox_left_motor_p->value();
    request->drivers_settings[0].i = ui->doubleSpinBox_left_motor_i->value();
    request->drivers_settings[0].d = ui->doubleSpinBox_left_motor_d->value();
    request->drivers_settings[0].max_total_i = ui->doubleSpinBox_left_motor_max_total_i->value();
    request->drivers_settings[0].k = ui->doubleSpinBox_left_motor_k->value();
    request->drivers_settings[0].b = ui->doubleSpinBox_left_motor_b->value();

    request->drivers_settings[1].p = ui->doubleSpinBox_right_motor_p->value();
    request->drivers_settings[1].i = ui->doubleSpinBox_right_motor_i->value();
    request->drivers_settings[1].d = ui->doubleSpinBox_right_motor_d->value();
    request->drivers_settings[1].max_total_i = ui->doubleSpinBox_right_motor_max_total_i->value();
    request->drivers_settings[1].k = ui->doubleSpinBox_right_motor_k->value();
    request->drivers_settings[1].b = ui->doubleSpinBox_right_motor_b->value();

    request->sensor_settings.accel_offset_x = ui->spinBox_mpu6050_accel_offset_x->value();
    request->sensor_settings.accel_offset_y = ui->spinBox_mpu6050_accel_offset_y->value();
    request->sensor_settings.accel_offset_z = ui->spinBox_mpu6050_accel_offset_z->value();
    request->sensor_settings.gyro_offset_x = ui->spinBox_mpu6050_gyro_offset_x->value();
    request->sensor_settings.gyro_offset_y = ui->spinBox_mpu6050_gyro_offset_y->value();
    request->sensor_settings.gyro_offset_z = ui->spinBox_mpu6050_gyro_offset_z->value();
...
void MotionWidget::on_write_config() {
    ...
    request->drivers_settings[0].motor_pinpwm = ui->spinBox_left_motor_pinPwm->value();
    request->drivers_settings[0].pluses_per_revolution = ui->spinBox_left_motor_pluses_per_revolution->value();
    request->drivers_settings[0].revolutions_per_minute = ui->spinBox_left_motor_revolutions_per_minute->value();
    ...
    request->drivers_settings[1].motor_pinpwm = ui->spinBox_right_motor_pinPwm->value();
    request->drivers_settings[1].pluses_per_revolution = ui->spinBox_right_motor_pluses_per_revolution->value();
    request->drivers_settings[1].revolutions_per_minute = ui->spinBox_right_motor_revolutions_per_minute->value();
    ...
    request->sensor_settings.pin_tx = ui->spinBox_lidar_pin_tx->value();
    request->sensor_settings.pin_rx = ui->spinBox_lidar_pin_rx->value();
    request->sensor_settings.pin_pwm = ui->spinBox_lidar_pin_pwm->value();
...
void MotionWidget::on_recv_motion_settings_service_response(uint64_t id, MotionSettingsSrv::Response::SharedPtr response) {
    ...
    if (response->mode == MotionService::Type::ReadParams) {
        ...
        ui->checkBox_spdPlan_enable->blockSignals(true);
        ui->checkBox_spdPlan_enable->setChecked(response->spd_plan_settings.enable);
        ui->checkBox_spdPlan_enable->blockSignals(false);
        ...
        ui->doubleSpinBox_left_motor_p->setValue(response->drivers_settings[0].p);
        ...
        ui->doubleSpinBox_right_motor_d->setValue(response->drivers_settings[1].d);
        ui->doubleSpinBox_right_motor_max_total_i->setValue(response->drivers_settings[1].max_total_i);
        ui->doubleSpinBox_right_motor_k->setValue(response->drivers_settings[1].k);
        ui->doubleSpinBox_right_motor_b->setValue(response->drivers_settings[1].b);

        ui->spinBox_mpu6050_accel_offset_x->setValue(response->sensor_settings.accel_offset_x);
        ui->spinBox_mpu6050_accel_offset_y->setValue(response->sensor_settings.accel_offset_y);
        ui->spinBox_mpu6050_accel_offset_z->setValue(response->sensor_settings.accel_offset_z);
        ui->spinBox_mpu6050_gyro_offset_x->setValue(response->sensor_settings.gyro_offset_x);
        ui->spinBox_mpu6050_gyro_offset_y->setValue(response->sensor_settings.gyro_offset_y);
        ...
        ui->spinBox_left_motor_pinPwm->setValue(response->drivers_settings[0].motor_pinpwm);
        ui->spinBox_left_motor_pluses_per_revolution->setValue(response->drivers_settings[0].pluses_per_revolution);
        ui->spinBox_left_motor_revolutions_per_minute->setValue(response->drivers_settings[0].revolutions_per_minute);
        ...
        ui->spinBox_right_motor_pinPwm->setValue(response->drivers_settings[1].motor_pinpwm);
        ui->spinBox_right_motor_pluses_per_revolution->setValue(response->drivers_settings[1].pluses_per_revolution);
        ui->spinBox_right_motor_revolutions_per_minute->setValue(response->drivers_settings[1].revolutions_per_minute);
        ...
        ui->spinBox_lidar_pin_pwm->setValue(response->sensor_settings.pin_pwm);
...
void MotionWidget::on_recv_motion_status_msg(const MotionStatusMsg::SharedPtr msg) {
    ...
    if (qFabs(msg->drivers_status[0].current_v) > minValue ||
        qFabs(msg->drivers_status[0].target_v) > minValue ||
        qFabs(msg->drivers_status[1].current_v) > minValue ||
        qFabs(msg->drivers_status[1].target_v) > minValue) {
        if (stop_plot_) {
            stop_plot_ = false;
        }
    }
    ...
    wheel_speed_customPlot_->graph(0)->addData(msg->stamp * 1e-9 - reference_seconds_, msg->drivers_status[0].current_v);
    wheel_speed_customPlot_->graph(1)->addData(msg->stamp * 1e-9 - reference_seconds_, msg->drivers_status[0].target_v);
    wheel_speed_customPlot_->graph(2)->addData(msg->stamp * 1e-9 - reference_seconds_, msg->drivers_status[1].current_v);
    wheel_speed_customPlot_->graph(3)->addData(msg->stamp * 1e-9 - reference_seconds_, msg->drivers_status[1].target_v);
    ...
    QString line = QString("t= %1\n"
                           "left_current_v= %2,  left_target_v= %3,  right_current_v= %4,  right_target_v= %5\n")
                       .arg(reference_seconds_, 0, 'f', 3)
                       .arg(msg->drivers_status[0].current_v, 0, 'f', 3)
                       .arg(msg->drivers_status[0].target_v, 0, 'f', 3)
                       .arg(msg->drivers_status[1].current_v, 0, 'f', 3)
                       .arg(msg->drivers_status[1].target_v, 0, 'f', 3);
...
void MotionWidget::_update_speed_percent_label(double linear_percent, double angular_percent) {
    ui->label_linear_speed_percent->setText(QString::number(linear_percent * 100, 'f', 1) + "%");
    ui->label_angular_speed_percent->setText(QString::number(angular_percent * 100, 'f', 1) + "%");
}
